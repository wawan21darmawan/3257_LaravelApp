...
@section('page_subtitle', 'Masukkan detail acara baru organisasimu.')

@section('content')
    <div class="bg-white p-8 rounded-[2.5rem] border border-slate-100 shadow-sm max-w-3xl">

        <!-- UBAH DISINI: admin.events.store menjadi organizer.events.store -->
        <form action="{{ route('organizer.events.store') }}" method="POST" enctype="multipart/form-data" class="space-y-6">
            ...
            <div>
                ...
                <input type="text" name="title" value="{{ old('title') }}" class="w-full px-5 py-4 bg-slate-50 rounded-2xl"
                    required>
            ...
            <div class="grid grid-cols-2 gap-6">
                <input type="datetime-local" name="date" value="{{ old('date') }}"
                    class="px-5 py-4 bg-slate-50 rounded-2xl">
                <input type="text" name="location" value="{{ old('location') }}" placeholder="Lokasi"
                    class="px-5 py-4 bg-slate-50 rounded-2xl">
            </div>
